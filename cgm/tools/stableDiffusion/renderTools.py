...
#bake projection texture
def bakeProjection(material, meshObj, resolution=(2048, 2048)):
    convertedFile = mc.convertSolidTx(
        # material.outColor. 
        "%s.outColor" % material,
        # name.	Set the name of the file texture node. 
        meshObj,
        #antiAlias(aa), Perform anti-aliasing on the resulting image
        antiAlias=1,
        #backgroundMode(bm),"shader" or 1: uses the default shader color.
        bm=1,
        #fillTextureSeams(fts), Specify whether or not to overscan the polygon beyond its outer edges, when creating the file texture, 
        # in order to fill the texture seams.Default is true.
        fts=0,
        #samplePlane(sp), 	Specify whether to sample using a virtual plane.
        sp=0,
        #shadows(sh), Specify whether to compute shadows when baking lighting
        sh=0,
        #alpha(al), Specify whether to compute the transparency when baking lighting.
        alpha=False,
        #doubleSided(ds), Specify whether the sampler should flip the surface normal if the sample point faces away from the camera.
        doubleSided=0,
        #componentRange(cr)
        componentRange=0,
...
def addImageToCompositeShader(shader, color, alpha):
    ...
    # important method remap alpha map
    ...
    mc.setAttr("%s.red[1].red_Position" % remapColor, 0.9)
...
# important method to remap alpha map of images
